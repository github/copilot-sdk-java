 ...
 *            when {@code true}, instructs the CLI to skip the permission check
 *            for this tool's calls; {@code null} or {@code false} uses the
...
public record ToolDefinition(@JsonProperty("name") String name, @JsonProperty("description") String description,
    ...
    /**
     * Creates a tool definition that skips the default permission check.
     * <p>
     * Use this factory method when your tool is safe to invoke without user
     * approval (e.g., read-only or idempotent operations). Setting
     * {@code skipPermission} to {@code true} instructs the CLI to bypass the
     * permission flow for this tool's calls.
     *
     * @param name
     *            the unique name of the tool
     * @param description
